...
def find_cct_for_all_load_levels(
    ...
    **kwargs
    ...
    if output_dir:
        ...
        # Generate timestamped filenames (simple implementation to avoid torch dependency)
        from datetime import datetime

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        csv_file = output_dir / f"cct_load_variation_results_{timestamp}.csv"
        df.to_csv(csv_file, index=False)
        logger.info(f"\nResults saved to: {csv_file}")

        # Also save as JSON for easier reading
        json_file = output_dir / f"cct_load_variation_results_{timestamp}.json"
...
Examples:
  ...
  python scripts/find_cct_load_variation.py --base-load-P 0.5 --base-load-Q 0.2
    ...
    # Check if case file has a load device, add/modify one if needed
    has_load, load_model, load_idx = check_smib_has_load(str(case_file))
    if not has_load:
        logger.info(f"No load device found in case file. Adding PQ load to bus 3...")
        try:
            modified_case_path = add_pq_load_to_smib_case(
                case_path=str(case_file),
                bus_idx=3,  # Bus 3 is the load/fault bus in SMIB
                p0=args.base_load_P,  # Use base load P from arguments
                q0=args.base_load_Q,  # Use base load Q from arguments
                output_path=None,  # Create temp file
            )
            logger.info(
                f"Added PQ load (P={args.base_load_P:.6f} pu, Q={args.base_load_Q:.6f} pu) to bus 3"
            )
            logger.info(f"Using modified case file: {modified_case_path}")
            case_file = Path(modified_case_path)
        except Exception as e:
            logger.warning(f"Could not add load to case file: {e}")
            logger.warning(f"Load variation may not work correctly if case has no load device.")
    else:
        logger.info(f"Found existing {load_model} load device (index {load_idx}) in case file")
        # Check if load has zero values - if so, modify it
        try:
            import andes
            import json

            # Quick check: read case file to see if load values are zero
            with open(case_file, "r") as f:
                case_data = json.load(f)

            load_data = case_data.get(load_model, {})
            if isinstance(load_data, dict):
                p0_list = load_data.get("p0", [])
                p0_value = (
                    p0_list[0]
                    if isinstance(p0_list, list) and len(p0_list) > 0
                    else (p0_list if not isinstance(p0_list, list) else 0.0)
                )
            elif isinstance(load_data, list) and len(load_data) > 0:
                p0_value = load_data[0].get("p0", 0.0)
            else:
                p0_value = 0.0

            # If load is zero, modify it
            if abs(p0_value) < 1e-6:
                logger.info(
                    f"Existing load has zero values (P={p0_value:.6f} pu). Modifying to P={args.base_load_P:.6f} pu, Q={args.base_load_Q:.6f} pu..."
                )
                modified_case_path = modify_case_file_load_setpoint(
                    case_path=str(case_file),
                    load_idx=load_idx or 0,
                    new_p=args.base_load_P,
                    new_q=args.base_load_Q,
                    load_model=load_model,
                    output_path=None,  # Create temp file
                )
                logger.info(f"Modified load values in case file: {modified_case_path}")
                case_file = Path(modified_case_path)
            else:
                logger.info(
                    f"Existing load has non-zero values (P={p0_value:.6f} pu). Using as-is."
                )
        except Exception as e:
            logger.warning(f"Could not check/modify load values: {e}")
            logger.warning(f"Proceeding with original case file. Load values may be zero.")
