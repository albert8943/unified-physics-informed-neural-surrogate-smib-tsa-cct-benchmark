...
def generate_training_data(
    ...
    if M_val is not None:
        ...
        elif isinstance(H_val, list) and len(H_val) == 3:
            H_range = tuple(H_val)
        elif isinstance(H_val, tuple) and len(H_val) == 3:
            H_range = H_val
        else:
            H_range = (
                H_val if isinstance(H_val, tuple) else None
            )  # Use case file default if invalid

    D_val = param_ranges.get("D", None)
    if D_val is None:
        D_range = None  # Use case file default
    elif isinstance(D_val, list) and len(D_val) == 3:
        D_range = tuple(D_val)
    elif isinstance(D_val, tuple) and len(D_val) == 3:
        D_range = D_val
    else:
        D_range = D_val if isinstance(D_val, tuple) else None  # Use case file default if invalid

    # Support both Pm variation (old) and load variation (new)
    Pm_val = param_ranges.get("Pm", None)
    if Pm_val is not None:
        if isinstance(Pm_val, list) and len(Pm_val) == 3:
            Pm_range = tuple(Pm_val)
        elif isinstance(Pm_val, tuple) and len(Pm_val) == 3:
            Pm_range = Pm_val
        else:
            Pm_range = Pm_val if isinstance(Pm_val, tuple) else None
    else:
        Pm_range = None
    ...
    if not use_pe_as_input:
        ...
        if input_method == "pe_direct":
            ...
            print(
                "[INFO] Model config uses 'pe_direct' input method. "
